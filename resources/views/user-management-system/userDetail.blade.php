...
@section('content')
    ...
    <div class="flex justify-center mx-auto p-4 mt-8">
        <div class="w-full mx-auto border border-gray-200 rounded-lg p-6 bg-white mt-8 shadow-xl">
            <div class="flex justify-center mb-8 h-14">
                <img src="{{ $user->image ? asset('storage/' . $user->image->url) : asset('assets/img/profile.png') }}"
                    alt="Profile Picture" class="w-20 h-20 rounded-full border-2 border-gray-300 object-cover">
            </div>
                ...
                @if ($user != null)
                    <div class="w-full px-3 mb-6 ">
                        <h5 class="text-xl font-bold mb-2">{{ $user['name'] }}
                            <span
                                class="text-xs font-semibold px-2.5 py-0.5 rounded ml-3
                    {{ isset($user['status']) && $user['status'] == 'active' ? 'bg-blue-100 text-blue-800' : 'bg-red-100 text-red-800' }}">
                        ...
                        <p class="text-gray-700">Department:
                            {{ $user->user_department?->department?->department ?? 'No Department' }}</p>
